...
def transfer_df_to_dict(study_parameters, params,scenario_list,load_path, pv_path):
    ...
    # Transfer remaining scenario parameters from df to dict
    for s in scenario_list:
        ...
        if 'en' in study_parameters.loc[s, 'arrangement']:
            params['study_parameters_mike']['tariffs']['parent'] = study_parameters.loc[s, 'parent']
        ...
        #todo MR!! Find 'all' and individual customer tariffs and transfer those - per scenario
        # Transfer cp pointers
        if 'cp' in study_parameters.columns:
            params['study_parameters_mike']['common_property_load_profile'] = 'cp'
            params['study_parameters_mike']['common_property_solar_profile'] = 'cp' # May need to be conditional??
        ...
        if 'en' in study_parameters.loc[s, 'arrangement']:
            params['study_parameters_mike']['central_solar_profile'] = 'total'
...
def main(base_path,project,study_name, override_output = ''):
    with open('mike_params1.json') as params_file:
...
if __name__ == "__main__":
    ...
    default_project = 'ceem_ui'
    default_study = 'ceem_ui_default'
